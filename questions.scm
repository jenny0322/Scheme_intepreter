...
(define (list-partitions total max-pieces max-value)
  (define (make-parts total max-pieces max-value current)
    (cond ((or (= total 0) (< max-value 1) (< max-pieces 0))'(nil nil))
      ((< total 0) '(current))
        (else (append (make-parts (- total max-value) max-value (- 1 max-pieces) '(max-value current))
                      (make-parts total (- max-value 1) max-pieces current)))))
  (make-parts total max-pieces max-value '()))
...
(define (tree-sums tree)
  (cond ((null? tree) 0)
    ((null? (children tree)) tree)
    (else (map tree-sums
      (map (lambda (subtree)(make-tree (+ (entry tree) (entry subtree))
        (children subtree))) (children tree))))))
