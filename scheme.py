...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try: 
            return self.fn(*python_args), None
        except TypeError as x : 
            raise SchemeError(x)

def convert(scheme_lst):
    if scheme_lst == nil:
        return []
    else:
        return [scheme_lst.first] + convert(scheme_lst.second)
...
class NuProcedure(LambdaProcedure):
    ...
    "*** YOUR CODE HERE ***"
    def apply(self, args, env):
        return args.map(lambda operand: Thunk(nil, operand, env))
...
class Thunk(LambdaProcedure):
    ...
    "*** YOUR CODE HERE ***"
    def get_actual_value(self, env):
        val= self.apply(nil, self.env)[0]
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        target = target.first
        if scheme_symbolp(target):
            vals = Pair(sec, vals.second)
            env.bindings[target] = do_lambda_form(vals, env)[0]
            return target, None
        else:
            raise SchemeError ('not a Scheme value')
    else:
...
def do_let_form(vals, env):
    ...
    for val in bindings:
        ...
        values = Pair(scheme_eval(val[1], env), values)
...
def do_or_form(vals, env):
    ...
    if len(vals) == 0 :
        return (scheme_false, None)
    for v in vals:
        if scheme_eval(v, env):
            return (scheme_eval(v, env), None)
    return (scheme_false, None)
...
def check_formals(formals):
    ...
    if scheme_listp(formals):
        repeated = convert(formals)
        for i in range(len(formals)) : 
            if not (scheme_symbolp(formals[i])) : 
                raise SchemeError
            repeated.remove(formals[i])
            if formals[i] in repeated: 
                raise SchemeError 
        return 
    else : 
        raise SchemeError 
